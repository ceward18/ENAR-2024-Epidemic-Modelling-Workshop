...
# Using a Stronger Prior for Gamma

The previously used prior puts 90% probability on the mean infectious period 
being between 4 and 6 days and is centered on 5 days.
...
Now, let's use a stronger prior

```{r}
bb_2 <- 2000
aa_2 <- 0.2 * bb_2
pgamma(1/4, aa_2, bb_2) - pgamma(1/6, aa_2, bb_2)
curve(dgamma(x, aa_2, bb_2))
```

We will also use a joint MH proposal for beta and gamma to account for the correlation 
...
system.time({
    compiled <- compileNimble(sirModelFit, myMCMC) 
    samples2 <- runMCMC(compiled$myMCMC, niter = 100000, nburnin = 50000, setSeed = 5)
})
...
hist(samples2[,'gamma'], freq = F, main =  'stronger prior', xlim = c(0.12, 0.26))
